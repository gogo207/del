...
public class AddDropLocationActivity extends ParentActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!sessionManager.getPickLt().equals("")) {
            currentLatitude = Double.parseDouble(sessionManager.getPickLt());  //전 화면에서 선택된 pickUp장소
            currentLongitude = Double.parseDouble(sessionManager.getPickLg());
        }
        vehicleUrl = sessionManager.getVehicleUrl();  //탈것 이미지 url
        vehicleName = sessionManager.getVehicleName();  //탈것 이름

        //이전 활동에서 번들의 모든 데이터를 가져 오는 것
        ...
        if (comingFrom != null && comingFrom.equals("signup")) {  //signup에서 현재 페이지로 넘어옴
    ...
    private void initViews() {
        ...
        if (comingFrom.equals("pick")) //pick이면 타이틀 pickuplocation으로 바꿈
    ...
    private void toggleFavAdrsList(final boolean isToShowFavAdrsLis) {
        ...
        if (isToShowFavAdrsLis) {
            Log.d(TAG, "toggleFavAdrsList: dddd"+isToShowFavAdrsLis);
